...
function JsonRpcApiProvider(ProviderType) {
    return class Provider extends ProviderType {
        ...
        async getBalance(address, blockTag, token) {
            if (!token || (0, utils_1.isAddressEq)(token, utils_1.L2_BASE_TOKEN_ADDRESS))
                return await super.getBalance(address, blockTag);
            try {
                return await typechain_1.IERC20__factory.connect(token, this).balanceOf(address, {
                    blockTag,
                });
            }
            catch {
        ...
        async getWithdrawTx(transaction) {
            ...
            (_a = tx.overrides).type ?? (_a.type = utils_1.EIP712_TX_TYPE);
            const baseToken = typechain_1.IBaseToken__factory.connect(utils_1.L2_BASE_TOKEN_ADDRESS, this);
            const populatedTx = await baseToken.withdraw.populateTransaction((0, ethers_1.toUtf8Bytes)(tx.to), tx.overrides);
        ...
        getRpcTransaction(tx) {
            ...
            if (tx.customData.factoryDeps) {
                result.eip712Meta.factoryDeps = tx.customData.factoryDeps.map((dep) => Array.from(ethers_1.ethers.getBytes(dep)));
            }
